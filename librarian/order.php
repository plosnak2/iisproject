        ...
        <div class="container" style="background-color:rgba(241,241,241,255); padding-top:20px">
            <div class="form-row">
                <div class="form-group col-md-12">
                    <h3>Hlasy za dokúpenie kníh v knižnici: <?php echo $library;?></h3>
                    <button type="button" onclick="window.location.href='./new_order.php?lib_name=<?php echo $library;?>'" class="btn btn-primary" style="margin-top:10px">Objednať inú knihu</button>
                    <hr>
                </div>
            </div>
            <table class="table table-striped table-dark" style="margin-bottom:0px">
                <thead class="thead-dark">
                    ...
                    <th scope="col">Počet Hlasov</th>
                    <th scope="col">Objednať</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                        $distinct_isbns = $db->get_unique_book_isbn($library);
                        while($dist_isbn = $distinct_isbns->fetch())
                        {
                            $book = $db->get_book($dist_isbn['book_isbn']);
                            $number_of_votes = $db->num_of_votes($dist_isbn['book_isbn'], $library);
                            echo "<tr>";
                            echo '<td> <img src="../images/books/'.$book['isbn'].'.png" style="width:10vw"/> </td>';
                            echo '<td style="vertical-align:middle"><b>'.$book['name'].'</b></td>';
                            echo '<td style="vertical-align:middle"><b>'.$number_of_votes['count'].'</b></td>';
                            echo '<td style="vertical-align:middle"><button type="button" onclick="window.location.href='."'./new_order.php?isbn=". $book['isbn']. "&lib_name=" . $library . "'" .'" class="btn btn-primary" style="margin-top:10px">Objednať</button></td>';
                            echo '</tr>';
                        }
                    ?>
                </tbody>
